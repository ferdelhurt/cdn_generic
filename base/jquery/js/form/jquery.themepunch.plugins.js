...
\**************************************/;
    ...
    a.fn.cssEngine = function(b, d) {
        ...
        var s = b;
        a.extend(r, b);
        if (r.display == "callbackHide") k = true; else if (r.display) h["display"] = r.display;
        e.cssOriginal(h);
        setTimeout(function() {
            e.cssOriginal(r);
    ...
    a.fn.cssFadeTo = function(b, c, d, e) {
        ...
        if (a.cssAnimateSupport()) {
            f.each(function(b, d) {
                d = a(d);
                if (d.data("displayOriginal") != d.cssOriginal("display") && d.cssOriginal("display") != "none") d.data("displayOriginal", d.cssOriginal("display") ? d.cssOriginal("display") : "block"); else d.data("displayOriginal", "block");
    ...
    // SWIPE FUNCTION
        ...
        return this.each(function() {
            function J() {
                var a = I();
                if (a <= 45 && a >= 0) return d; else if (a <= 360 && a >= 315) return d; else if (a >= 135 && a <= 225) return e; else if (a > 45 && a < 135) return g; else return f;
            }
            function I() {
                var a = y.x - z.x;
                var b = z.y - y.y;
